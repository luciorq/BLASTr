...
primer check below runs in R, so neither needs a tool.
...
carries the primers you expect. `search_primers_on_fq()` streams plain
or gzip-compressed FASTQ files and counts, per primer, how many reads
contain it. IUPAC degenerate bases are supported and, by default, both
strands are searched. The check runs in R, so this chunk is evaluated
here on a small synthetic library shipped with the package (300 reads:
150 with the MiFish-U forward primer on the forward strand, 100 with it
on the reverse strand, and 50 with a COI primer instance).

```{r}
#| label: search-primers
#| eval: true
...
lineages from the NCBI E-utilities (`efetch`), directly over HTTPS.
Requests are batched (up to 100 Tax IDs per request):
