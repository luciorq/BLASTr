...
# Upstream repository is `heronoh/BLASTr`; development happens on the
# `luciorq/BLASTr` fork (git remotes `upstream` / `origin`).
github_org := 'heronoh'
...
# =============================================================================
# BLASTr specific Tasks
# =============================================================================

# Rebuild the pinned conda tool environment (BLAST+) from scratch
reinstall-tool-envs:
  #!/usr/bin/env bash
  \builtin set -euo pipefail;
  R -q -s -e 'devtools::load_all(quiet=TRUE);install_dependencies(verbose="output", force=TRUE);';

# Run the taxonomy client validation against the real Entrez Direct tools
test-edirect-equivalence:
  #!/usr/bin/env bash
  \builtin set -euo pipefail;
  NOT_CRAN=true R -q -s -e 'devtools::load_all(quiet=TRUE);testthat::test_local(load_package="none", filter="eutils-vs-edirect");';
